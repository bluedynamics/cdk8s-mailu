...
export class RspamdConstruct extends Construct {
  ...
  constructor(scope: Construct, id: string, props: RspamdConstructProps) {
    ...
    this.pvc = new kplus.PersistentVolumeClaim(this, 'pvc', {
      ...
      storage: Size.gibibytes(parseInt(config.storage?.rspamd?.size?.replace('Gi', '') || '5')),
        ...
        ? {
          cpu: {
            request: kplus.Cpu.millis(
              parseInt(config.resources.rspamd.requests?.cpu?.replace('m', '') || '100'),
            ),
            limit: config.resources.rspamd.limits?.cpu
              ? kplus.Cpu.millis(parseInt(config.resources.rspamd.limits.cpu.replace('m', '') || '500'))
              : undefined,
          },
          memory: {
            request: this.parseMemorySize(config.resources.rspamd.requests?.memory || '512Mi'),
            limit: config.resources.rspamd.limits?.memory
              ? this.parseMemorySize(config.resources.rspamd.limits.memory)
              : undefined,
          },
        }
        : undefined,
